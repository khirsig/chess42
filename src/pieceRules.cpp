...
/*   Updated: 2022/08/05 15:11:09 by khirsig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int kingMove(Board &chessBoard, int pieceX, int pieceY, int xAdd, int yAdd) {
    ...
    // It checks if the piece on the target square a rook and if any of the involved pieces have
    // already moved. if (chessBoard.square[pieceY][pieceX + xAdd].piece != nullptr)
    if (abs(xAdd) > 0 && yAdd == 0 &&
        ...
        while ((incr == 1 && x <= xAdd) || (incr == -1 && x >= xAdd)) {
            if (chessBoard.square[pieceY][pieceX + x].piece != nullptr && x != xAdd) return (-1);
    ...
    if (chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece &&
        ...
        return (-1);
    // The last two cases check if the kings tries to move further than one field in his normal
    // movement.
    if (abs(xAdd) > 1 || abs(yAdd) > 1) return (-1);
    if (abs(xAdd) != 1 && abs(yAdd) != 1) return (-1);
    return (KING_NORMAL);
}
...
static bool rookMove(Board &chessBoard, int owner, int pieceX, int pieceY, int xAdd, int yAdd) {
    ...
    if (xAdd != 0 && yAdd != 0) return (false);
    if (chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece &&
        chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece->getOwner() ==
            chessBoard.square[pieceY][pieceX].piece->getOwner())
        return (false);
    // if (chessBoard.square[pieceX + xAdd][pieceY + yAdd].piece && chessBoard.square[pieceX +
    // xAdd][pieceY + yAdd].piece->getOwner() == owner) 	return (false);

    // We set both the x increment value and the y increment value according if our xAdd and yAdd
    // goal are positive or negative. This way we can iterate through the loop later in correct
    ...
    while (((yIncr == 1 && y <= yAdd) || (yIncr == -1 && y >= yAdd) || (yIncr == 0)) &&
           ((xIncr == 1 && x <= xAdd) || (xIncr == -1 && x >= xAdd) || (xIncr == 0))) {
        if (chessBoard.square[pieceY + y][pieceX + x].piece != nullptr && (x != xAdd || y != yAdd))
            return (false);

        if ((xIncr == 1 && x <= xAdd) || (xIncr == -1 && x >= xAdd)) x += xIncr;
        if ((yIncr == 1 && y <= yAdd) || (yIncr == -1 && y >= yAdd)) y += yIncr;
    }
    return (true);
}

static bool knightMove(Board &chessBoard, int owner, int pieceX, int pieceY, int xAdd, int yAdd) {
    if (!((abs(xAdd) == 2 && abs(yAdd) == 1) || (abs(xAdd) == 1 && abs(yAdd) == 2))) return (false);
    if (chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece &&
        chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece->getOwner() ==
            chessBoard.square[pieceY][pieceX].piece->getOwner())
        return (false);
...
static bool bishopMove(Board &chessBoard, int owner, int pieceX, int pieceY, int xAdd, int yAdd) {
    ...
    if (chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece &&
        ...
        return (false);
    if (xAdd == 0 || yAdd == 0) return (false);
    if (abs(xAdd) != abs(yAdd)) return (false);
    // if (chessBoard.square[pieceX + xAdd][pieceY + yAdd].piece && chessBoard.square[pieceX +
    // xAdd][pieceY + yAdd].piece->getOwner() == owner) 	return (false);
...
static bool pawnMove(Board &chessBoard, int pieceX, int pieceY, int xAdd, int yAdd,
                     bool lookForCheck) {
    ...
    if (abs(yAdd) > 2 || abs(xAdd) > 1) return (false);
    if (chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece && abs(xAdd) != 1) return (false);
    if (chessBoard.square[pieceY][pieceX].piece->getOwner() == WHITE_P) {
        if (yAdd > 0) return (false);
        if (pieceY != 6 && abs(yAdd) > 1) return (false);
        ...
        if (yAdd < 0) return (false);
        if (pieceY != 1 && abs(yAdd) > 1) return (false);
        if (yAdd == 1 && abs(xAdd) == 1 && chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece &&
            chessBoard.square[pieceY + yAdd][pieceX + xAdd].piece->getOwner() == WHITE_P)
            return (true);
    }
    if (abs(yAdd) != 1 && abs(xAdd) >= 1) return (false);
    ...
    if (abs(xAdd) == 0 && lookForCheck) return (false);
    return (true);
}

bool isMovePossible(Board &chessBoard, int pieceX, int pieceY, int xAdd, int yAdd,
                    bool lookForCheck) {
    ...
    // Every piece has its own move function that checks if the fields it wants to move through are
    // possible. The King has some special cases with castling.
    switch (chessBoard.square[pieceY][pieceX].piece->getType()) {
        case PAWN:
            if (pawnMove(chessBoard, pieceX, pieceY, xAdd, yAdd, lookForCheck)) return (true);
            break;
        case BISHOP:
            if (bishopMove(chessBoard, pieceOwner, pieceX, pieceY, xAdd, yAdd)) return (true);
            break;
        case KNIGHT:
            if (knightMove(chessBoard, pieceOwner, pieceX, pieceY, xAdd, yAdd)) return (true);
            break;
        case ROOK:
            if (rookMove(chessBoard, pieceOwner, pieceX, pieceY, xAdd, yAdd)) return (true);
            break;
        case QUEEN:
        ...
        case KING:
            ...
            if (ret == KING_NORMAL) return (true);
            if (ret == KING_SHORT_CASTLE) return (true);
            if (ret == KING_LONG_CASTLE) return (true);
            return (false);
            break;
    }
    return (false);
    ...
    for (int y = 0; y < 8; ++y) {
        for (int x = 0; x < 8; ++x) {
            ...
            if (chessBoard.square[y][x].piece &&
                ...
                // Looks if any of those pieces could make a theoretical move to the position of the
                // king. We need the piece that could give check and its position and also the x and
                // y value it would need to make that move. Last bool should be true because we're
                // looking for a check, not moving.
                if (isMovePossible(chessBoard, x, y, chessBoard.kingPosX[player] - x,
                                   chessBoard.kingPosY[player] - y, true))
                    return (true);
            }
        }
    }
