...
/*   Updated: 2022/08/05 15:10:36 by khirsig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float getPieceSquareValue(ChessPiece *piece, int x, int y, bool endgame) {
    if (endgame) return (egFieldValues[piece->getType()][y * 8 + x]);
            ...
            if (chessBoard.square[pieceY][pieceX].piece &&
                ...
                for (int targetY = 0; targetY < 8; ++targetY) {
                    for (int targetX = 0; targetX < 8; ++targetX) {
                        if (isMovePossible(chessBoard, pieceX, pieceY, targetX - pieceX,
                                           targetY - pieceY, false))
                            mobility++;
                    }
                }
            }
...
float calculateBoard(Board &chessBoard, int player, bool relative) {
            ...
            if (chessBoard.square[y][x].piece) {
                ...
                if (player == BLACK_P) {
                    if (pieceOwner == player) {
                        ...
                        playerSquareValue +=
                            getPieceSquareValue(chessBoard.square[y][x].piece, x,
                                                getOppositeSquare(y), chessBoard.endgame);
                        ...
                        opponentSquareValue += getPieceSquareValue(chessBoard.square[y][x].piece, x,
                                                                   y, chessBoard.endgame);
                        if ((pieceType == KNIGHT || pieceType == BISHOP) && !pieceMoved)
                    ...
                    if (pieceOwner == player) {
                        playerPieceValue += chessBoard.square[y][x].piece->getValue();
                        playerSquareValue += getPieceSquareValue(chessBoard.square[y][x].piece, x,
                                                                 y, chessBoard.endgame);
                        if ((pieceType == KNIGHT || pieceType == BISHOP) && !pieceMoved)
                            playerSquareValue -= 50 * DEVELOPMENT_FACTOR;
                    } else {
    ...
    if (relative == false)
        return (((playerPieceValue + (playerSquareValue / 10)) -
                 (opponentPieceValue + (opponentSquareValue / 10))) /
                100);

    if (player == WHITE_P) {
        if (chessBoard.castled[WHITE_P] == -1) playerSquareValue -= 60 * CASTLE_FACTOR;
        if (chessBoard.castled[BLACK_P] == -1) opponentSquareValue -= 60 * CASTLE_FACTOR;
    } else {
        if (chessBoard.castled[BLACK_P] == -1) playerSquareValue -= 60 * CASTLE_FACTOR;
        if (chessBoard.castled[WHITE_P] == -1) opponentSquareValue -= 60 * CASTLE_FACTOR;
    }
    if (chessBoard.endgame == false && playerPieceValue < endGameThreshold &&
        opponentPieceValue < endGameThreshold) {
    ...
    float boardValue = (playerPieceValue + playerSquareValue + playerMobility) /
                       (opponentPieceValue + opponentSquareValue + opponentMobility);
    return (boardValue);
}
...
ChessPiece *movePiece(Board &chessBoard, int pieceX, int pieceY, int targetX, int targetY) {
    ...
    if (chessBoard.square[targetY][targetX].piece->getType() == KING) {
        if (targetX == 7 && deletedPiece && deletedPiece->getType() == ROOK &&
            deletedPiece->getHasMoved() == false && pieceHasMoved == false) {
...
void revertMovePiece(Board &chessBoard, int pieceX, int pieceY, int targetX, int targetY,
                     ChessPiece *deletedPiece, int player) {
    int castledSave = -1;
...
float depthCalculation(Board &chessBoard, int movingPlayer, int calcPlayer, int currentDepth,
                       int totalDepth, float alpha, float beta, bool relative) {
    float value;
                        ...
                        // && !possibleMoveCheck(chessBoard, pieceX, pieceY, targetX, targetY))
                        {
                            if (currentDepth < totalDepth) {
                                ChessPiece *deletedPiece =
                                    movePiece(chessBoard, pieceX, pieceY, targetX, targetY);
                                if (totalDepth == currentDepth + 1) {
                                    value = calculateBoard(chessBoard, calcPlayer, relative);
                                    revertMovePiece(chessBoard, pieceX, pieceY, targetX, targetY,
                                                    deletedPiece, movingPlayer);
                                    return (value);
                                }
                                float ret = depthCalculation(chessBoard, (movingPlayer + 1) % 2,
                                                             calcPlayer, currentDepth + 1,
                                                             totalDepth, alpha, beta, relative);
...
float getBestMove(Board *chessBoard, int pieceX, int pieceY, int player,
                  std::vector<Move> &allMoves, int depth, bool relative) {
    ...
    for (int y = 0; y < 8; ++y) {
        for (int x = 0; x < 8; ++x) {
            ...
            // && !possibleMoveCheck(chessBoard, pieceX, pieceY, x, y))
            {
                ...
                float currentMove;
                if (player == WHITE_P)
                    currentMove = depthCalculation(*chessBoard, otherPlayer, player, 1, depth,
                                                   -INFINITY, INFINITY, relative);
                else
                    currentMove = depthCalculation(*chessBoard, otherPlayer, player, 1, depth,
                                                   -INFINITY, INFINITY, relative);
...
void executeAIMove(Data &data, Board &chessBoard, int player) {
    if (data.aiThreadMove.wait_for(std::chrono::milliseconds(1)) == std::future_status::ready) {
        ...
        ChessPiece *deletedPiece = movePiece(chessBoard, bestMove.getStartX(), bestMove.getStartY(),
                                             bestMove.getTargetX(), bestMove.getTargetY());
        ...
        if (chessBoard.castled[player] != chessBoard.moveTurn) {
            ...
            if (chessBoard.kingPosX[player] == 6)
                data.lastMove[0] =
                    &chessBoard
                         .square[chessBoard.kingPosY[player]][chessBoard.kingPosX[player] - 1];
            else
                data.lastMove[0] =
                    &data.chessBoard
                         .square[chessBoard.kingPosY[player]][chessBoard.kingPosX[player] + 1];
            data.lastMove[1] =
                &data.chessBoard.square[chessBoard.kingPosY[player]][chessBoard.kingPosX[player]];
        }
