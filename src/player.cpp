...
/*   Updated: 2022/08/05 15:11:10 by khirsig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
                        ...
                        if (chessBoard.square[y][x].piece &&
                            ...
                            if (!possibleMoveCheck(chessBoard, x, y, innerX, innerY)) {
                                if ((chessBoard.square[innerY][innerX].piece &&
                                     chessBoard.square[innerY][innerX].piece->getOwner() !=
                                         player) ||
...
void placePiece(Data &data, int player) {
    ...
    if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT) && data.grabbedPiece != nullptr) {
        ...
        x /= size;
        y /= size;
        // Checks if the move from the position of the grabbed piece to the position of the mouse
        // cursor is possible. To do so it needs a pointer to the piece, the grabbed piece position
        // and the x and y value that would need to be added to it to make that move. Last bool
        // should only be true if we're looking for a check.
        if (!data.chessBoard.checkmate && data.turn == data.grabbedPiece->getOwner() &&
            isMovePossible(data.chessBoard, data.grabbedPiecePosX, data.grabbedPiecePosY,
                           x - data.grabbedPiecePosX, y - data.grabbedPiecePosY, false)) {
            // If the move is theoretically possible, checks if the square our piece lands on does
            // not have any piece of the same owner on it, or is empty.
            if ((data.chessBoard.square[y][x].piece != nullptr &&
                 data.chessBoard.square[data.grabbedPiecePosY][data.grabbedPiecePosX]
                         .piece->getOwner() != data.chessBoard.square[y][x].piece->getOwner()) ||
                data.chessBoard.square[y][x].piece == nullptr ||
                (data.chessBoard.square[data.grabbedPiecePosY][data.grabbedPiecePosX]
                         .piece->getType() == KING &&
                 (x == 7 || x == 0))) {
                ChessPiece *deletedPiece =
                    movePiece(data.chessBoard, data.grabbedPiecePosX, data.grabbedPiecePosY, x, y);

                // Now after the move has been made, checks if there is any check for the player
                // that made the move.
                if (lookForCheck(data.chessBoard, player))
                    revertMovePiece(data.chessBoard, data.grabbedPiecePosX, data.grabbedPiecePosY,
                                    x, y, deletedPiece, player);
                else {
                    // The square from - to the move happened are saved for different color in board
                    // drawing.
                    if (data.chessBoard.castled[player] != data.chessBoard.moveTurn) {
                        ...
                        if (data.chessBoard.kingPosX[player] == 6)
                            data.lastMove[0] =
                                &data.chessBoard.square[data.chessBoard.kingPosY[player]]
                                                       [data.chessBoard.kingPosX[player] - 1];
                        else
                            data.lastMove[0] =
                                &data.chessBoard.square[data.chessBoard.kingPosY[player]]
                                                       [data.chessBoard.kingPosX[player] + 1];
                        data.lastMove[1] =
                            &data.chessBoard.square[data.chessBoard.kingPosY[player]]
                                                   [data.chessBoard.kingPosX[player]];
                    }
                    data.moveNbr++;
        ...
        // After everything is over looks for checks on both players. If one king is in check sets
        // the bool for board drawing.
        toggleCheckBothPlayers(data.chessBoard);
        if (lookForCheckmate(data.chessBoard)) data.chessBoard.checkmate = true;
    }
}

void grabPiece(Data &data) {
    // If the left mouse button is pressed we set our grabbedPiece pointer to the piece that is on
    // the current field. If there is no piece, nothing happens. If we already have one piece
    // grabbed nothing happens.
